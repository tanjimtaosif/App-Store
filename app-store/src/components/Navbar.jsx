        ...
        navItems.map(({ path, label }) => (
            <li key={path} onClick={() => setIsOpen(false)}>
                <NavLink
                    to={path}
                    className={({ isActive }) =>
                        `relative px-2 text-base font-medium transition-all duration-200 hover:text-transparent hover:bg-clip-text hover:bg-gradient-to-br hover:from-[#632EE3] hover:to-[#9F62F2] ${isActive
    ...
    return (
        <nav className="w-full bg-white shadow-sm relative">
            <div className="max-w-[1440px] mx-auto flex justify-between items-center py-5 px-5 lg:px-10">
                ...
                {/* CTA */}
                <Link
                    to="https://github.com/tanjimtaosif"
